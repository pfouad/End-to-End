				...
				if ent2.is_class("ISP_PORT"): #if the current entity is an ISP_PORT
					...
					#check for De(Mux) or patch panel
						...
						try:
							...
							a_end_isp_design = addedInJob(ent2,"End Equipment")+": "+checkValue(ent2.ISPA_SECTION_F_CODE)  + " ; " + checkValue(chassis.ISPA_NAME)+ " ; " + checkValue(parent.ISPA_NAME) + " - "+ checkValue(chassis.gdm_ea_attr_21) + " | " + checkValue(chassis.gdm_ea_attr_20)
							if portmc == True:
				...
				if ent2.is_class("ISP_PORT"): #if ent2 is an isp port then
					...
					#check for De(Mux) or patch panel
						...
						try:
							...
							z_end_isp_design = addedInJob(ent2,"End Equipment")+": "+checkValue(ent2.ISPA_SECTION_F_CODE) + " ; " + checkValue(chassis.ISPA_NAME) +" ; " + checkValue(parent.ISPA_NAME) + " - " + checkValue(chassis.gdm_ea_attr_21) + " | " + checkValue(chassis.gdm_ea_attr_20)
	...
	def generateVisio(self, schemaData):
		...
		try:
					...
					for k in range(0,len(schemaData[0])):
						if (schemaData[0][j].ASite == schemaData[0][j].ZSite) and j == 0:
							if (schemaData[0][j].ASiteEndEquip != None and schemaData[0][j].ASiteEndEquip != ""):
								...
								if tupleA[0]== 'End Equipment_new' or tupleA[0] == 'End Equipment':
									if tupleAT[2] == 'IP':
							...
							if (schemaData[0][j].ZEndEquip != None and schemaData[0][j].ZEndEquip != ""):
								...
								if tupleZ[0]== 'End Equipment_new' or tupleZ[0] == 'End Equipment':
									if tupleZT[2] == 'IP':
								...
								if (schemaData[0][k].ZEndEquip != None and schemaData[0][k].ZEndEquip != ""):
									...
									if tupleZ[0]== 'End Equipment_new' or tupleZ[0] == 'End Equipment':
										if tupleZT[2] == 'IP':
											...
											self._placeItem(self.left, "Nortel OM6500", tupleZ[2])
							break
				usageZ = tupleZT[0].partition("- ")
				self._placeEquipment(self.center, "DWDM/IP System", usageZ[2])
			...
			if midDrop == True:
				...
				tupleAend = schemaData[0][length-1].ASiteEndEquip.partition("| ")
				tupleZend = schemaData[0][length-1].ZEndEquip.partition("| ")
				...
				if ipA == True and ipZ == True:
							...
							if tupleAend[2] == 'Transport':
								...
								if usageAend[2].upper() == usageZi[2].upper():
									if i == length-2:
										...
										temp = schemaData[0][length-2]
										schemaData[0][length-2] = schemaData[0][i]
										schemaData[0][i] = temp
										useZ = True
									...
									if i == length-2:
										...
										temp = schemaData[0][length-2]
										schemaData[0][length-2] = switchSides(i)
										schemaData[0][i] = temp
										useZ = True

							elif tupleZend[2] == 'Transport':
								usageZend = tupleZend[0].partition("- ")
								shape2.Text = usageZend[2]
								usageZend[2].replace(" ", "")
					...
					if useA == True and useZ == True:
						...
						usageZ1 = tupleZ1[0].partition("- ")
						usageZ1[2].replace(" ", "")
						tupleAN = schemaData[0][length-2].ASiteEndEquip.partition("| ")
						usageAN = tupleAN[0].partition("- ")
						usageAN[2].replace(" ", "")
						if length > 4:
							for i in range(2,length-3):
								for j in range(2,length-3):
									...
									elif usageAN[2].upper() == usageAj[2].upper(): 
										temp = schemaData[0][length-3]
										schemaData[0][length-3] = switchSides(j)
										schemaData[0][j] = temp
									elif usageAN[2].upper() == usageZj[2].upper():
										temp = schemaData[0][length-3]
										schemaData[0][length-3] = schemaData[0][j]
										schemaData[0][j] = temp
									elif usageAN[2].upper() == usageAi[2].upper(): 
										temp = schemaData[0][length-3]
										schemaData[0][length-3] = switchSides(i)
										schemaData[0][i] = temp
									elif usageAN[2].upper() == usageZi[2].upper():
										temp = schemaData[0][length-3]
										schemaData[0][length-3] = schemaData[0][i]
										schemaData[0][i] = temp 
									elif usageAN[2].upper() == usageAi[2].upper() and i == length-3:
										schemaData[0][i] = switchSides(i)
									else:
										if usageAi[2].upper() == usageZj[2].upper() and j < i and j != i-1:
											temp = schemaData[0][j+1]
											schemaData[0][j+1] = schemaData[0][i]
											schemaData[0][i] = temp
										elif usageZi[2].upper() == usageZj[2].upper() and j< i and j != i-1:
											temp = schemaData[0][j+1]
											schemaData[0][j+1] = switchSides(i)
											schemaData[0][i] = temp	
							...
							if schemaData[0][j].ASite == schemaData[0][k].ASite and j!=k:
								...
								if (tupleAT[2] == 'IP' or tupleZT[2] == 'IP' or tupleAkT == 'IP' or tupleZkT == 'IP'):
									...
									if self.left["previousShape"] == None:
										...
										if (schemaData[0][k].ASiteEndEquip != None and schemaData[0][k].ASiteEndEquip != ""):
											tupleAk = schemaData[0][k].ASiteEndEquip.partition(": ")
											if tupleAk[0]== 'End Equipment_new' or tupleAk[0] == 'End Equipment':
												if tupleAkT[2] == 'IP':
													self._placeItem(self.left, "Router", tupleAk[2])
												elif tupleAkT[2] == 'Transport':
													self._placeItem(self.left, "Nortel OM6500", tupleAk[2])
										if (schemaData[0][k].ZEndEquip != None and schemaData[0][k].ZEndEquip != ""):
											...
											if tupleZk[0]== 'End Equipment_new' or tupleZk[0] == 'End Equipment':
												if tupleZkT[2] == 'IP':
